...
class SpeechEngine:
    def __init__(self, text_mode: bool = False, language: str = "en", tts_rate: int = 150):
        ...
        if os.environ.get("OPENAI_API_KEY"):
            try:
                ...
                print("[SPEECH] Using OpenAI Whisper API (cloud, most accurate)")
            except ImportError:
                print("[SPEECH] openai package not installed, falling back to local Whisper")

        if not self._use_openai_stt:
            import whisper
            print(f"[SPEECH] Loading local Whisper model ({WHISPER_MODEL})...")
    ...
    def listen(self) -> str:
        ...
        print(f"[SPEECH] Listening (silence={silence_timeout}s, hint={hint})")
        ...
        print(f"[SPEECH] Processing {duration:.1f}s of audio")
    ...
    def listen_after_interrupt(self) -> str:
        ...
        print("[SPEECH] Continuing listen after interrupt (no beep)")
        ...
        print(f"[SPEECH] Heard (interrupt): '{text}'")
        return text
