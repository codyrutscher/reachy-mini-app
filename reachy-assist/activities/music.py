...
class MusicPlayer:
    ...
    def _init_song_db(self):
        ...
        try:
            ...
            logger.error("Song DB init error: %s", e)
    ...
    def scan_songs_folder(self):
        ...
        conn.close()
        if added:
            logger.info("Scanned songs folder: %d new songs added", added)
        return added
    ...
    def play_file(self, path):
        ...
        logger.info("Playing: %s", os.path.basename(path))

    def play_melody(self, name):
        notes = MELODIES.get(name)
        if not notes:
            logger.warning("Unknown melody: %s", name)
            return
    ...
    # ── Song favorites / request memory ──────────────────────────

    def log_song_request(self, query: str, patient_id: str = "default") -> None:
        """Remember that the patient requested this song/artist/genre."""
        try:
            from memory import db_supabase as db
            if db.is_available():
                db._execute(
                    "INSERT INTO song_favorites (patient_id, request_text) VALUES (%s, %s)",
                    (patient_id, query),
                )
        except Exception:
            pass

    def get_favorites(self, patient_id: str = "default", limit: int = 20) -> list[str]:
        """Get the patient's most-requested songs/queries."""
        try:
            from memory import db_supabase as db
            if db.is_available():
                rows = db._execute(
                    "SELECT request_text, COUNT(*) as cnt FROM song_favorites "
                    "WHERE patient_id=%s GROUP BY request_text ORDER BY cnt DESC LIMIT %s",
                    (patient_id, limit), fetch=True,
                )
                if rows:
                    return [r["request_text"] for r in rows]
        except Exception:
            pass
        return []

    def suggest_favorite(self, patient_id: str = "default") -> str | None:
        """Suggest playing a favorite song. Returns suggestion text or None."""
        favs = self.get_favorites(patient_id, limit=5)
        if not favs:
            return None
        import random
        fav = random.choice(favs)
        return f"Would you like me to play '{fav}'? You've enjoyed that before."
...
def _write_wav(path, notes, sample_rate=44100):
    ...
    logger.debug("Generated: %s", os.path.basename(path))
