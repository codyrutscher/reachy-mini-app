...
class GratitudeSession:
    ...
    # ── Gratitude chain — build on past sessions ─────────────────

    def get_past_gratitudes(self, limit: int = 30) -> list[str]:
        """Get past gratitude entries to build a growing chain."""
        try:
            import memory.db_supabase as _db
            if _db.is_available():
                facts = _db.get_facts(self.patient_id, category="gratitude")
                return [f["fact"].replace("Grateful for: ", "") for f in (facts or [])][:limit]
        except Exception:
            pass
        return []

    def get_chain_context(self) -> str:
        """Build context about the gratitude chain for the LLM."""
        past = self.get_past_gratitudes(limit=10)
        if not past:
            return ""
        count = len(past)
        recent = past[:3]
        items = "; ".join(recent)
        return (
            f"(The patient has shared {count} gratitudes over past sessions. "
            f"Recent ones: {items}. You might reference these — "
            f"'Last time you mentioned...' or 'Your gratitude list is growing!')"
        )

    def start_with_chain(self) -> str:
        """Start a session that references past gratitudes."""
        self.active = True
        self.answers = []
        self._current_ask = 0
        past = self.get_past_gratitudes(limit=20)
        count = len(past)

        if count == 0:
            return self.start()

        recent = past[0] if past else ""
        return (
            f"(Start a gratitude practice. The patient has done {count} gratitudes before. "
            f"Their most recent was: '{recent}'. Say something like: "
            f"'Let's add to your gratitude collection — you've shared {count} things so far! "
            f"Last time you mentioned {recent}. What are you grateful for today?')"
        )
